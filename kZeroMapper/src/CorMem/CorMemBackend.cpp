...
bool CorMemBackend::ReadWritePhysical( uint64_t physicalAddress, void* buffer, uint64_t bytes, bool write )
{
	...
	bool CorMemBackend::ValidateCr3WithNtoskrnl( uint64_t cr3 )
	{
		if ( !m_Ntoskrnl )
			return false;
	...
	bool CorMemBackend::QueryPml4( uint64_t* value )
	{
		if ( !value )
			return false;
		...
		*value = 0;
		...
		{
			LOG_SEC( "[-] - [CorMem] QueryPml4 failed to map low 16MB gle=%lu", GetLastError( ) );
			return false;
		}
		...
		if ( !readOk )
			LOG_SEC( "[-] - [CorMem] QueryPml4 exception reading mapped low memory", 0 );
		...
		if ( stubOffset != 0xFFFFFFFF )
		{
			...
			LOG_SEC( "[*] - [CorMem] QueryPml4 low stub cr3=0x%llx offset=0x%X", rawCr3, stubOffset );

			if ( ValidateCr3WithNtoskrnl( rawCr3 ) )
			{
				m_Pml4Cache = rawCr3;
				*value = rawCr3;
				LOG_SEC( "[*] - [CorMem] QueryPml4 validated pml4=0x%llx", *value );
				return true;
			}
			...
			for ( int i = 0; i < 5; ++i )
			{
				...
				if ( ValidateCr3WithNtoskrnl( kptiCandidates[ i ] ) )
				{
					...
					*value = kptiCandidates[ i ];
					LOG_SEC( "[*] - [CorMem] QueryPml4 KPTI kernel cr3=0x%llx (variant %d, user=0x%llx)",
						*value, i, rawCr3 );
					return true;
				}
			}

			LOG_SEC( "[-] - [CorMem] Low stub CR3 and KPTI variants all failed validation", 0 );
		}

		LOG_SEC( "[*] - [CorMem] Scanning low 16MB for CR3 candidates", 0 );

		const uint64_t validationVa = m_Ntoskrnl;
		if ( !validationVa )
		{
			LOG_SEC( "[-] - [CorMem] No validation VA (ntoskrnl=0)", 0 );
			return false;
		}
		...
		ULONG tested = 0;
		{
			...
			for ( size_t i = 0; i < qwordCount; ++i )
			{
				...
				*value = candidate;
				LOG_SEC( "[*] - [CorMem] QueryPml4 found pml4=0x%llx at offset=0x%llx (tested=%lu)",
					*value, i * 8, tested );
				return true;
			}
		}

		LOG_SEC( "[-] - [CorMem] QueryPml4 failed to find PML4 (tested=%lu candidates)", tested );
		return false;
	}

bool CorMemBackend::PageEntryToPhysicalAddress( uint64_t entry, uint64_t* physicalAddress )
{
...
bool CorMemBackend::VirtualToPhysicalWithCr3( uint64_t cr3, uint64_t virtualAddress, uint64_t* physicalAddress )
{
	if ( !physicalAddress )
		return false;
...
bool CorMemBackend::VirtualToPhysicalByTableWalk( uint64_t virtualAddress, uint64_t* physicalAddress )
{
	if ( !physicalAddress )
		return false;
	...
	if ( !QueryPml4( &pml4 ) )
	{
		LOG_SEC( "[-] - [CorMem] VirtualToPhysicalByTableWalk failed QueryPml4 va=0x%llx", virtualAddress );
		return false;
	}

	if ( !VirtualToPhysicalWithCr3( pml4, virtualAddress, physicalAddress ) )
	{
		LOG_SEC( "[-] - [CorMem] VirtualToPhysicalByTableWalk failed va=0x%llx pml4=0x%llx", virtualAddress, pml4 );
		return false;
	}
...
bool CorMemBackend::ReadWriteVirtual( uint64_t address, void* buffer, size_t size, bool write )
{
...
bool CorMemBackend::WriteMemory( uint64_t address, const void* buffer, size_t size )
{
	...
	if ( !result )
		LOG_SEC( "[-] - [CorMem] WriteMemory failed va=0x%llx size=0x%llx", address, size );
